...
async def handle_study(bot: Bot, event: MessageEvent, state: T_State):
...
async def got_study(bot: Bot, event: MessageEvent, state: T_State):
    result = state['result']
...
async def _del_lib(bot: Bot, event: MessageEvent, state: T_State):
    arg = str(event.get_message()).split()
    if arg:  # 空元素返回
        if arg[1] not in ['show', 'del']:
            ...
            if arg[1] == 'show':
                if isinstance(event, PrivateMessageEvent):
                    imgmsg = await makeLibImg(StudyPath=StudyPath)
                    await lib_manage.finish(imgmsg)
                else:
...
@get_march.handle()
async def _get_march(bot: Bot, event: MessageEvent):
    try:
