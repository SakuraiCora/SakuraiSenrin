...
async def info_get(bot: Bot, event:GroupMessageEvent):
    ...
    if isinstance(event, GroupMessageEvent) and isinstance(args, int):
        ...
        save_pic = await get_info_card(QQnum, member_info['nickname'], member_info['sex'], member_info['title'], f"LV.{member_info['level']}", join_time_format)
        await info.finish(MessageSegment.image(save_pic))
    else:
